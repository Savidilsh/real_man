...
dual_arm_msgs::JointPos joint_pos_msg;
...
int main(int argc, char** argv)
{
    ...
    // 获取臂类型参数，默认为右臂
    std::string arm_type;
    private_nh_.param<std::string>("arm_type", arm_type, "r");
    
    // 根据臂类型设置话题前缀
    std::string arm_prefix = arm_type + "_arm";
    std::string controller_name = arm_type + "_arm_controller";
    ...
    // 使用参数化的话题名称
    get_ArmError = nh.subscribe(arm_prefix + "/rm_driver/ArmError", 100, getArmError_Callback);
    system_error_clean_pub = nh.advertise<std_msgs::Empty>(arm_prefix + "/rm_driver/Clear_System_Err", 300);
    target_pub = nh.advertise<dual_arm_msgs::JointPos>(arm_prefix + "/rm_driver/JointPos", 300);
    
    // 使用参数化的控制器名称
    // Server server(nh, controller_name + "/follow_joint_trajectory", boost::bind(&execute, _1, &server), false);
    Server server(nh, arm_prefix + "/follow_joint_trajectory", boost::bind(&execute, _1, &server), false);
