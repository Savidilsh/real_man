...
class HungarianMatcher(nn.Module):
    ...
    def __init__(
        ...
        Params:
            cost_class: This is the relative weight of the binary classification error in the matching cost
    ...
    def memory_efficient_forward(self, outputs, targets):
        ...
        bs, num_queries = outputs["logit"].shape[:2]
        ...
        for b in range(bs):
            ...
            out_mask = outputs["mask"][b].T  # [num_queries, num_points]
            tgt_mask = target["mask"].to(out_mask)  # [num_targets, num_points]
    ...
    def forward(self, outputs, targets):
        ...
        Params:
            outputs: This is a dict that contains at least these entries:
                 "logit": Tensor of dim [batch_size, num_queries, num_classes] with the classification logits
                 "mask": Tensor of dim [batch_size, num_queries, H_pred, W_pred] with the predicted masks

            targets: This is a list of targets (len(targets) = batch_size), where each target is a dict containing:
                 "caption": List[str] containing the captions
                 "mask": Tensor of dim [num_target_boxes, H_gt, W_gt] containing the target masks
